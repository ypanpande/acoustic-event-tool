...
from readrcfd_class import Read_rcfd
...
# ==============================================================================
    ...
    def init_gui(self):
        ...
        self.create_sourcefile_display()
        self.create_eventfile_display()
        self.create_event_figure_display()
        self.create_acc_figure_display()

    def create_top_menu_bar(self):
        self.topbar_frame = tk.Frame(
            self.root, height=60, width=1630, relief='ridge', borderwidth=1)
        self.topbar_frame.place(x=5, y=5, width=1630, height=60)

        ttk.Button(self.topbar_frame, text="FFT", command=self.event_fft).place(
            x=5, y=10, width=90, height=30)

        ttk.Button(self.topbar_frame, text="Acc Info", command=self.event_acc_info).place(
            x=105, y=10, width=90, height=30)

        ttk.Button(self.topbar_frame, text="Loc Info", command=self.event_loc_info).place(
            x=205, y=10, width=90, height=30)

        ttk.Button(self.topbar_frame, text="Character", command=self.event_character).place(
            x=305, y=10, width=90, height=30)

        ttk.Button(self.topbar_frame, text="Convert AES data",
                   command=self.convert_AES_data).place(x=405, y=10, width=120, height=30)

        ttk.Button(self.topbar_frame, text="Convert ACC data",
                   command=self.convert_ACC_data).place(x=535, y=10, width=120, height=30)
...
#        self.prog.place(x = 665, y = 10, width = 200, height = 30)
    ...
    def create_sourcefile_display(self):
        self.source_display_frame = tk.Frame(
            self.root, height=820, width=430, relief='ridge', borderwidth=1)
        self.source_display_frame.place(x=5, y=70, width=430, height=820)

        bsource = ttk.Button(self.source_display_frame,
                             text="Source folder", command=self.rcdf_input)
        bsource.place(x=5, y=5, width=90, height=25)

        self.lsource = ttk.Label(
            self.source_display_frame, text=self.default_source_folder, background="white")
        self.lsource.place(x=100, y=5, width=325, height=25)

        ttk.Label(self.source_display_frame, text="Rcdf files:").place(
            x=5, y=40,  width=420, height=25)
    ...
    def create_eventfile_display(self):
        event_display_frame = tk.Frame(
            self.root, height=820, width=270, relief='ridge', borderwidth=1)
        event_display_frame.place(x=445, y=70, width=270, height=820)

        ttk.Label(event_display_frame, text="Event files:").place(
            x=5, y=5,  width=260, height=25)
    ...
    def create_event_figure_display(self):
        self.event_figure_display_frame = tk.Frame(
            self.root, height=400, width=910, relief='ridge', borderwidth=1)
        self.event_figure_display_frame.place(
            x=725, y=70, width=910, height=400)

    def create_acc_figure_display(self):
        self.acc_figure_display_frame = tk.Frame(
            self.root, height=400, width=910, relief='ridge', borderwidth=1)
        self.acc_figure_display_frame.place(
            x=725, y=490, width=910, height=400)
...
# ================================================================================
    ...
    def create_event_fft(self):
        ef = self.get_event_data(
            self.file_path, self.current_efile)  # data form dict
        efd = Character(ef, 0.004)
        self.af2 = Figure(figsize=(16, 8), dpi=100)
        self.af2.suptitle('FFT of event: {}'.format(
            self.current_efile), fontsize=12)

        for i, k in enumerate(sorted(efd._data.keys())):
            f = efd.AmpFre_FFT(efd._data[k])
            aa = self.af2.add_subplot(2, 3, i+1)
            aa.plot(np.array(f['freq'])/1000, f['Amp'],
                    color='b', linewidth=1, label=k)
...
#            aa.set_xlim([0, 125000])
    ...
    def embed_event_fft(self):
        ...
        canvas = FigureCanvasTkAgg(
            self.af2, master=self.event_fft_canvas_frame)
        canvas.draw()
...
# ========================================================================================
    def event_acc_info(self):
        ...
        acc.title('Acceleration info Window for Event {}'.format(
            self.current_efile))
        #acc_data = self.get_acc_data()
        event_time = datetime.datetime.strptime(
            self.current_efile[-26:], '%d_%m_%Y_%H_%M_%S_%f')
        ad = ACC(self.df_acc, event_time)
        acc_info = ad.get_sub_acc_value()

        ttk.Label(acc, text='Event: {}'.format(self.current_efile), font=(
            'Helvetica', 13)).place(x=10, y=10, width=370, height=40)
        ttk.Label(acc, text='Time = {}'.format(acc_info[2]), font=(
            'Helvetica', 12)).place(x=8, y=55, width=370, height=30)

        ttk.Label(acc, text='Angle = {:3.0f}'.format(acc_info[0]), font=(
            'Helvetica', 12)).place(x=8, y=90, width=370, height=30)
        ttk.Label(acc, text='Rotate = {}'.format(acc_info[1]), font=(
            'Helvetica', 12)).place(x=8, y=125, width=370, height=30)
        ttk.Label(acc, text='a_r = {}'.format(acc_info[3]), font=(
            'Helvetica', 12)).place(x=8, y=160, width=370, height=30)

        ttk.Label(acc, text='a_x = {}'.format(acc_info[4]), font=(
            'Helvetica', 12)).place(x=8, y=195, width=370, height=30)
        ttk.Label(acc, text='a_y = {}'.format(acc_info[5]), font=(
            'Helvetica', 12)).place(x=8, y=230, width=370, height=30)
        ttk.Label(acc, text='a_z = {}'.format(acc_info[6]), font=(
            'Helvetica', 12)).place(x=8, y=265, width=370, height=30)

# ===========================================================================================
    def event_loc_info(self):
        locw = tk.Toplevel(self.topbar_frame, height=280, width=500)
        locw.title('Location info Window for Event {}'.format(
            self.current_efile))
        ...
        ttk.Label(locw, text='Event: {}'.format(self.current_efile), font=(
            'Helvetica', 13)).place(x=10, y=10, width=490, height=40)
        if seq6s['x'] == None:
            ttk.Label(locw, text='Sequential6s: None Results', font=(
                'Helvetica', 12)).place(x=8, y=55, width=490, height=30)
        else:
            ttk.Label(locw, text='Sequential6s: x = {0:5.0f}, y = {1:5.0f}, res = {2:5.3f}'.format(
                seq6s['x'], seq6s['y'], seq6s['res']), font=('Helvetica', 12)).place(x=8, y=55, width=490, height=30)

        if seq4s['x'] == None:
            ttk.Label(locw, text='Sequential4s: None Results', font=(
                'Helvetica', 12)).place(x=8, y=90, width=490, height=30)
        else:
            ttk.Label(locw, text='Sequential4s: x = {0:5.0f}, y = {1:5.0f}, res = {2:5.3f}'.format(
                seq4s['x'], seq4s['y'], seq4s['res']), font=('Helvetica', 12)).place(x=8, y=90, width=490, height=30)

        if geiger6s['x'] == None:
            ttk.Label(locw, text='Geiger6s: None Results', font=(
                'Helvetica', 12)).place(x=8, y=130, width=490, height=30)
        else:
            ttk.Label(locw, text='Geiger6s: x = {0:5.0f}, y = {1:5.0f}, res = {2:5.3f}'.format(
                geiger6s['x'], geiger6s['y'], geiger6s['res']), font=('Helvetica', 12)).place(x=8, y=130, width=490, height=30)

        if geiger4s['x'] == None:
            ttk.Label(locw, text='Geiger4s: None Results', font=(
                'Helvetica', 12)).place(x=8, y=165, width=490, height=30)
        else:
            ttk.Label(locw, text='Geiger4s: x = {0:5.0f}, y = {1:5.0f}, res = {2:5.3f}'.format(
                geiger4s['x'], geiger4s['y'], geiger4s['res']), font=('Helvetica', 12)).place(x=8, y=165, width=490, height=30)
...
# ========================================================================================
    ...
    def event_character(self):
        ...
        chw.title('Character Info Window for Event {}'.format(
            self.current_efile))

        df = Character(pd.DataFrame(self.ef))
        onset = df.Onset_time()
        ttk.Label(chw, text='Event: {}'.format(self.current_efile), font=(
            'Helvetica', 15)).place(x=80, y=10, width=1200, height=40)
        ttk.Label(chw, text='Onset time: {}'.format(onset), font=(
            'Helvetica', 12)).place(x=8, y=55, width=1200, height=30)
        ttk.Label(chw, text='Time domain:', font=('Helvetica', 12)
                  ).place(x=8, y=95, width=1200, height=30)

        for k, v in enumerate(list(df._data)):
            d = df.Amp_Time_Character(df._data[v])
            ttk.Label(chw, text='{0}:  Max_Amp(t) = {1:5.0f}, time(max) = {2:5.3f}, Energy = {3:.2e}, Energy(2.5ms) = {4:.2e}, ZeroCrossf(2.5ms) = {5:5.1f}, rise_time = {6:5.0f}, RA = {7:5.0f}'.format(
                v, d['maxAmp_t'], d['time_peak'], d['Energy'], d['Energy25'], d['ZeroCrossf'], d['rise_time'], d['RA']), font=('Helvetica', 11)).place(x=8, y=135+35*k, width=1200, height=30)

        ttk.Label(chw, text='Frequency domain:', font=('Helvetica', 12)).place(
            x=8, y=350, width=1200, height=30)
        for key, val in enumerate(list(df._data)):
            dd = df.AmpFre_FFT(df._data[val])
            e = df.Amp_Fre_Character(dd['Amp'], dd['freq'])
            ttk.Label(chw, text='{0}:  Max_Amp(f) = {1:5.0f}, fre_peak = {2:5.3f}, fre_centroid = {3:5.1f}, fre_wpeak = {4:5.1f}, Power = {5:.2e}, PartialPower(%) = [{6:.1f}, {7:.1f}, {8:.1f}, {9:.1f}, {10:.1f}]'.format(
                val, e['maxAmp_f'], e['fre_peak'], e['fre_centroid'], e['fre_wpeak'], e['Power'], *e['PartialPower']), font=('Helvetica', 11)).place(x=8, y=385+35*key, width=1200, height=30)
...
# ==============================================================================
    ...
    def rcdf_event_input(self, event):
        self.current_sfile = self.sdisplay.get(
            "insert linestart", "insert lineend")
        self.file_path = os.path.join(
            self.source_folder_path, self.current_sfile)
        event_list = self.get_listOfEvent(self.file_path)
...
# ======================================================================================
    ...
    def create_acc_plot(self):
        ...
        a.plot(df_AES['time'], df_AES['event'], 'ko',
               linewidth=1, markersize=3, label='event')
        a.set_title('3 axis Acceleration of file: {}'.format(
            self.current_sfile), fontsize=12)
...
# ====================================================================================
    ...
    def create_event_plot(self):
        self.current_efile = self.edisplay.get(
            "insert linestart", "insert lineend")
        self.ef = self.get_event_data(
            self.file_path, self.current_efile)  # data form dict
        ...
        for i, k in enumerate(sorted(self.ef.keys())):
            aa.plot(time, np.array(self.ef[k])+i*offset,
                    color=color[i], linewidth=1, label=k)
